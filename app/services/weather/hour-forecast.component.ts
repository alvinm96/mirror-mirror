    ...
    this.weatherService.getHourlyForecast()
      .then((res) => {
        for(let i = 0; i < num; i++) {
          ...
          let time = (res.hourly_forecast[i].FCTTIME.hour % 12 || 12) + ":" + res.hourly_forecast[i].FCTTIME.min;
          let ampm = res.hourly_forecast[i].FCTTIME.ampm;
          let condition = res.hourly_forecast[i].icon;
          let temperature = res.hourly_forecast[i].temp.english;

          this.periods[this.periods.length++] = new Period(condition, temperature, time, ampm);
